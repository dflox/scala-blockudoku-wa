htmx.onLoad(() => {
    document.querySelectorAll('.tile-marker-wrapper').forEach(block => {
        block.addEventListener('mouseenter', () => {
            ...
            block.dataset.previewvalid0?.split(',').forEach(id => document.getElementById(id)?.classList.add('preview-valid'));
            block.dataset.previewinvalid0?.split(',').forEach(id => document.getElementById(id)?.classList.add('preview-invalid'));
        });
